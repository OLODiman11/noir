package com.olodiman11.noir.objects.actions.insider;

import com.olodiman11.noir.Handler;
import com.olodiman11.noir.gamemodes.Mode.Roles;
import com.olodiman11.noir.objects.actions.ActionButton;
import com.olodiman11.noir.states.GameState.Actions;

public class InsideJobButton extends ActionButton{

	public InsideJobButton(Handler handler) {
		super(handler);
	}

	@Override
	public void setupTexture() {
		texture = handler.getAssets().getRoles().get(Roles.Insider.getText())[3][0];
		selectedImg = handler.getAssets().getRoles().get(Roles.Insider.getText())[3][1];
	}

	@Override
	public void setupXnY() {
		x = x0 + handler.getGameState().getPreviewX() + 15 * ratio;
		y = y0 + handler.getGameState().getPreviewY() + 500 * ratio;
	}

	@Override
	public void processButtonPress() {
		handler.getGameState().setCurrAction(Actions.InsideJob);
		for(int row = 0; row < handler.getGameState().getNumRows(); row++) {
			for(int col = 0; col < handler.getGameState().getNumCols(); col++) {
				if(handler.getGameState().getMap()[row][col].getEv() != null) {
					handler.getGameState().getMap()[row][col].setActive(true);
				}
			}
		}
	}

}
