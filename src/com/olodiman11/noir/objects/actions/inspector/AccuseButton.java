...
public class AccuseButton extends ActionButton {

	public AccuseButton(Handler handler) {
		super(handler);
	...
	public void setupTexture() {
		texture = handler.getAssets().getRoles().get("inspector")[2][0];
		selectedImg = handler.getAssets().getRoles().get("inspector")[2][1];
		disabledImg = handler.getAssets().getRoles().get("inspector")[2][2];
	}

	@Override
	public void setupXnY() {
		x = x0 + handler.getGameState().getPreviewX() + 31 * ratio;
		y = y0 + handler.getGameState().getPreviewY() + 342 * ratio;
	}

	@Override
	public void processButtonPress() {
		handler.getGameState().setCurrAction(Actions.Accuse);
